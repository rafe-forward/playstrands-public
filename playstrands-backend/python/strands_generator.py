...
def generate_board(words):
    while True:
        ...
        if res:
            ...
            words2 = [item for item in words if item not in words1 and item != words[0]]
            side1_paths = generate_side1(board, side1, words1)
            side2_paths = generate_side2(board, side2, words2)

        if len(side2_paths) == len(words2) and side1_paths and side2_paths:
            return {
                "board": board,
                "spanagram_path": path,
                "side1_words": words1,
                "side2_words": words2,
                "side1_paths": side1_paths,
                "side2_paths": side2_paths
            }
...
if __name__ == "__main__":
    result = generate_board(["springtime", "showers", "robins", "puddles", "blossoms", "buds", "pollen"])
    print("Spanagram Path:", result["spanagram_path"])
    print("Side 1 Words & Paths:", list(zip(result["side1_words"], result["side1_paths"])))
    print("Side 2 Words & Paths:", list(zip(result["side2_words"], result["side2_paths"])))
